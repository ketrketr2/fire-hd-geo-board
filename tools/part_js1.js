...
RENDER.v0 = () => {
  ...
  <div class="card hero rv"><h2>Kindleは今、<em>どう語られているか</em>。<br><span style="font-size:18px;color:var(--ink2)">市場の現状・世間の声・AI6面の回答を、実データで1画面に。</span></h2>
    ...
    <div class="pillars">
      ...
      <div class="pl" onclick="go('v4')"><span class="arrow">→</span><div class="n">03 / AI</div><div class="t">${AI.measured ? 'AIの言及率 ' + pct(avgM, 0) + '・第一想起 ' + pct(avgF, 0) : 'AI6面 × ' + AI.queries.length + '本を計測待ち'}</div><div class="b v">${AI.measured ? pct(avgM, 0) : '計測待ち'}</div><div class="m">${AI.measured ? 'カテゴリ質問（' + AI.expect_cells + 'セル）でKindleに触れた率。第一想起1位は ' + (first ? esc(first.label) + ' ' + pct(first.rate, 0) : '—') + '。自社（Amazon）引用率 ' + pct(avgO, 0) : 'ChatGPT / Gemini / Claude / Perplexity / AI Overview / AIモード。Secrets登録後、初回ラウンドで自動反映'}</div></div>
    </div></div>
