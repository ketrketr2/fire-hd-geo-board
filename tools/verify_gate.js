/* 暗号化ゲートの復号ラウンドトリップ検証。鍵は環境変数 KINDLE_GATE_KEY（"id:pw"）から取り、リポジトリには書かない。 */
const fs = require('fs');
const { chromium } = require('playwright');
const PW_EXE = process.env.PW_CHROME || '/opt/pw-browsers/chromium';
const LAUNCH = fs.existsSync(PW_EXE) ? { executablePath: PW_EXE } : {};
const KM = process.env.KINDLE_GATE_KEY || '';
if (!KM.includes(':')) { console.error('KINDLE_GATE_KEY 未設定（"id:pw"）'); process.exit(1); }
...
(async () => {
  ...
  await p.click('#b'); await p.waitForTimeout(2000);
  ...
  await p.fill('#p', GPW);
  await p.click('#b'); await p.waitForTimeout(4500);
  const ok = await p.evaluate(() => ({ main: (document.querySelector('#main') || document.body).innerHTML.length, title: document.title, marker: document.documentElement.outerHTML.indexOf('KINDLE_BOARD') >= 0 }));
