...
RENDER.v8 = () => {
  ...
  <div class="card s12 rv" style="padding:16px 20px"><div class="ct"><h3>データ連携ステータス — 何が実測で、何がサンプルか</h3>${tagOf('live')}<span class="sub">このボードは「無い数値を作らない」。連携すると同じ場所に実値が入る</span><span class="hb" onclick="help('connect')">?</span></div>
    <div class="kpi">
      ...
      <div class="k orange"><div class="l">サンプル（連携で置換）</div><div class="v"><span data-cu="${cnt('sample')}">0</span><small>系統</small></div><div class="d">販売実績・量販店チャネル・Kids+会員</div></div>
...
RENDER.v9 = () => {
  ...
  const moves = [
    ...
    {n: '02', t: 'YouTubeを「AIが読む一次情報」に', s: 'AI引用の第1位ソースはYouTube（日本でも1位・p32）', d: '公式・量販店・レビュアー動画に字幕・チャプター・説明文（価格・型番・比較表）を整備。「タブレット おすすめ 2026」「安いタブレット」「子供用タブレット」の上位動画を狙う。Fire HD 10レビューの1位が『Google Playの入れ方』である状態を変える。', kpi: 'YouTube上位20本のうち公式/協力動画の本数', tag: 'オフサイト'},
    {n: '03', t: '公式ページを「1問1答・結論ファースト」に', s: m ? `自社（Amazon）引用率 ${pct(avgO, 0)}` : '資料p35: 引用される公式の型', d: 'モデル比較・選び方・「Google Playは使えるか」「何ができるか」の各質問に1URLで答える。価格・重量・RAM・解像度・対応アプリを本文HTML（表）に。PDF・画像内の情報は読まれない。', kpi: 'A/B族クエリでの自社引用率', tag: 'オンサイト'},
    ...
    {n: '05', t: '「Google Playが使えない」への正面回答', s: m && cLock ? `囲い込み・自由度の懸念率 ${pct(cLock.share, 0)}（好意${cLock.pos}／懸念${cLock.neg}文）` : 'Fire HD 10レビュー1位が「Google Playを入れる方法」（V3）', d: 'Amazonアプリストアで何が使えて何が使えないかを、アプリ名レベルの一覧で公式に出す。「YouTubeは？Netflixは？LINEは？」に1URLで答える。非公式導入の解説記事が一次情報になっている状態を、公式の正確な情報で置き換える。', kpi: 'H族の懸念文比率／「使えない」誤情報件数', tag: 'コンテンツ'},
    ...
    {n: '07', t: 'セール文脈の一次情報を公式発で', s: '需要の山はPD・BF・年末年始・新生活・サマー（V1/V2）', d: 'セール日程・価格の一次情報をプレスリリース＋公式ページで先出しし、まとめ記事の引用元を公式にする。「タブレット セール いつ」でのAI回答の出典を追う。', kpi: 'セール系クエリの自社引用率', tag: '一次情報'},
...
/* ---------- ヘルプ（指標定義） ---------- */
const HELP = {
  idx: ['AI指標の<em>定義</em>', `<ul><li><b>生成率</b>：質問に対してAIが回答を返した率（AI Overviewは表示されない質問がある）。</li><li><b>言及率（G2）</b>：Fireの名前が回答本文に出た率。分母は<b>出現期待クエリ</b>（カテゴリ型A・ペルソナ型B・ネガ一般H）— 指名型・比較型は「名前が出て当然」なので除外（ファネル原則）。</li><li><b>第一想起率（G3）</b>：言及があった回答のうち、最初に挙がったブランドがFireだった率。</li><li><b>自社引用率</b>：出典リンクのうち amazon.co.jp / aboutamazon.jp 等の比率。残りが第三者。</li><li><b>平均引用数（G1・露出機会）</b>：1回答あたりの出典リンク数。多い面ほど「引用で戦える」。</li><li><b>推薦転換率</b>：その媒体が引用された回答のうち、Fireが第一想起になった率（資料p34）。</li></ul><div class="formula">言及率 = Fire言及セル ÷ 出現期待セル ／ 第一想起率 = 第一想起セル ÷ Fire言及セル</div><p>検出はブランド辞書（Fire HD 8／Fire HD 10／Fire Max 11／Fireタブレット 等）をNFKC正規化・長い順に照合。Fire TV Stickは端末ブランド（タブレット）に数えない。Kindle（電子書籍リーダー）は別ブランドとして数えます。極性は文単位の辞書判定（好意語／懸念語）で、回答全文は無加工で保存・表示しています。</p>`],
  sample: ['<em>サンプル</em>の扱い', `<p>販売台数・売上・チャネル比率・Kids+会員・ファネルは社内データ連携前のため、<b>固定シードで生成した設計サンプル</b>です。実データには一切基づきません。SAMPLEリボン・点線枠・「サンプル」タグで必ず区別しています。</p><p>接続すると（データ連携ビュー参照）同じカードに実値が入り、リボンが消えます。市場統計・価格・トレンド・AI計測などは出典付きの実測です。</p>`],
  ...
  shelf: ['Amazonの棚の<em>読み方</em>', `<p>Amazon.co.jp で「タブレット」を検索した1ページ目を、その日の表示のまま採録しています（順位＝カテゴリのベストセラー順位、★とレビュー数＝商品カードの表示、「過去1か月」＝Amazonが表示する購入点数レンジ）。</p><p>棚の順位は<b>自社の販売力</b>、★は<b>満足度</b>、レビュー数は<b>累積の母数</b>。順位が高くても★が競合に負けていれば、AIやレビューサイトでの語られ方はいずれ効いてきます。DataForSEO Merchant を接続すると同じ表が自動更新になります。</p>`],
  kakaku: ['価格.comの<em>読み方</em>', `<p>価格.comの「タブレットPC」カテゴリの人気売れ筋ランキング（推定販売数ベース）を、その週の集計のまま採録しています。</p><p>重要なのは順位そのものより<b>Fireが1製品も登録されていない</b>という構造です。比較サイトは購買検討の中間地点であり、AIが参照する第三者ソースでもあるため、ここに存在しないことは「比較文脈での不在」を意味します。</p>`],
