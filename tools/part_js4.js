...
const HELP = {
  ...
  shelf: ['Amazonの棚の<em>読み方</em>', `<p>Amazon.co.jp で「電子書籍リーダー」を検索した1ページ目を、その日の表示のまま採録しています（順位＝カテゴリのベストセラー順位、★とレビュー数＝商品カードの表示、「過去1か月」＝Amazonが表示する購入点数レンジ）。</p><p>棚の順位は<b>自社の販売力</b>、★は<b>満足度</b>、レビュー数は<b>累積の母数</b>。順位が高くても★が競合に負けていれば、AIやレビューサイトでの語られ方はいずれ効いてきます。DataForSEO Merchant を接続すると同じ表が自動更新になります。</p>`],
  kakaku: ['価格.comの<em>読み方</em>', `<p>価格.comの「電子書籍リーダー」カテゴリの人気売れ筋ランキング（推定販売数ベース）を、その週の集計のまま採録しています。</p><p>重要なのは順位そのものより<b>Kindleが1製品も登録されていない</b>という構造です。比較サイトは購買検討の中間地点であり、AIが参照する第三者ソースでもあるため、ここに存在しないことは「比較文脈での不在」を意味します。</p>`],
  yt: ['YouTubeの<em>読み方</em>', `<p>AIが回答をつくるとき、動画では圧倒的にYouTubeが引用されます（米国AIOの引用シェア1位、日本でも5AI合算で引用1位・他の動画サイトの約200倍／資料p32）。AIは音声ではなく<b>書き起こし・説明文・字幕</b>を読みます。</p><p>だからここでは「誰が上位にいるか」を見ます。上位が第三者ばかりなら、AIが読むKindleの一次情報は第三者が書いていることになります。打ち手は、上位チャンネルとの連携（提供・監修）と、公式動画の字幕・チャプター・説明文の整備です。</p>`],
