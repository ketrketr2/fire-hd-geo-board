...
鍵素材は環境変数 KINDLE_GATE_KEY（"id:pw" を : で連結した1文字列）。
方式: PBKDF2-SHA256 200,000回 + AES-256-GCM + gzip（toyota-car-board / gotemba-geo-board 流用）。
復号後の検証マーカーは KINDLE_BOARD。
"""
import base64
import gzip
import os
...
km = os.environ.get('KINDLE_GATE_KEY')
if not km or ':' not in km:
    sys.exit('KINDLE_GATE_KEY 未設定（"id:pw" 形式。値はGitHub Secretsのみに置く）')
SRC = os.environ.get('SRC', 'plain.html')
DST = os.environ.get('DST', 'index_new.html')
doc = open(SRC, 'rb').read()
if b'KINDLE_BOARD' not in doc:
    sys.exit('plain.html に検証マーカー KINDLE_BOARD がありません')
